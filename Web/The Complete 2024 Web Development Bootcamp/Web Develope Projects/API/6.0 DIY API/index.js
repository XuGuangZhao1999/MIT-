...
app.post("/jokes", (req, res) => {
  ...
  let joke = {id: jokes.length, jokeText: text, jokeType: type};
  jokes.push(joke);
  res.json(joke);
});
//5. PUT a joke
app.put("/jokes/:id", (req, res) => {
  let {text, type} = req.body;
  let joke = {id: parseInt(req.params.id), jokeText: text, jokeType: type};
  let index = jokes.findIndex((item) => item.id === joke.id);
  if(-1 !== index){
    jokes[index] = joke;
  }
  res.json(joke);
...
app.patch("/jokes/:id", (req, res) => {
  let {text, type} = req.body;
  let id = parseInt(req.params.id);
  if(-1 !== jokes.findIndex((joke)=> joke.id === id)){
    if(text){
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(-1 !== index){
    [jokes[index], jokes[jokes.length - 1]] = [jokes[jokes.length - 1], jokes[index]];
    jokes.pop();
    res.sendStatus(200);
  }
  res.sendStatus(404);
...
app.delete("/all", (req, res) => {
  let userkey = req.query.masterKey;
